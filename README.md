...
<tr><td><b>Lives where you do</b></td><td>Telegram, Discord, Slack, Email, and CLI — all from a single gateway process. Voice memo transcription, cross-platform conversation continuity.</td></tr>
...
Hermes has two entry points: start the terminal UI with `hermes`, or run the gateway and talk to it from Telegram, Discord, Slack, or Email. Once you're in a conversation, many slash commands are shared across both interfaces.
...
# YAML mode: specify custom config
hermes -q "Use playwright-page-capture for page_id=baidu_poc config=/path/to/my-config.yaml"

# URL mode: direct URL, no config file needed
hermes -q "Use playwright-page-capture for page_id=https://www.baidu.com feishu_chat_id=oc_xxxx"
```
