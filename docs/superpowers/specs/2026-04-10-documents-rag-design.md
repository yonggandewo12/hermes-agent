# Documents RAG MemoryProvider — Design Spec

## 1. Overview

**Name**: `documents` MemoryProvider plugin
**Type**: 独立的 pluggable memory provider
**Purpose**: 自动召回本地文档知识库内容，在 Agent 对话中注入相关片段
**Mechanism**: 文档索引 → 独立 SQLite 存储 → `prefetch()` 混合检索召回

`documents` provider 的职责边界如下：
- 负责固定目录 `~/.hermes/docs/` 的启动扫描
- 负责提供 `attach_document(path=...)` 工具，处理对话内即时上传并索引
- 负责文档解析、chunk、建库、检索与 `prefetch()` 注入
...
└──────────┬───────────┘
           ▼
┌──────────────────────┐
│     TextChunker      │  ← 优先按段落切分，超长再按字符数切分并 overlap
└──────────┬───────────┘
           ▼
┌──────────────────────┐
...
┌──────────────────────────────────────────────┐
│                 DocRetriever                 │
│      FTS5 top-N candidate recall + HRR rerank│
└──────────────────────────────────────────────┘
                       │
                       ▼
...
## 5. Text Chunking Rules

chunking 规则明确如下：
- 优先按空行/段落边界切分
- 若段落超过 `chunk_size`，再按字符数切分
- `chunk_size` 和 `chunk_overlap` 均按字符计，不按 token 计
- 默认：`chunk_size=500`，`chunk_overlap=50`
- PDF 提取后先做文本清洗：合并多余空白、去掉明显空行、保留正文顺序

## 6. Retrieval and Encoding Flow

### 文本编码

对 query 和 chunk 使用完全同构的编码流程：
1. 文本归一化：转小写、合并多余空白、移除明显标点噪音
2. token 化：按非字母数字边界切分，丢弃长度为 1 的 token
3. 每个 token 执行 `encode_atom(token, dim=1024)`
...
在 provider `initialize()` 时扫描 `~/.hermes/docs/` 下所有支持的文档。
...
| token 级 `encode_atom` 后 bundle | 比“整段文本直接 encode”更接近可解释的文本 HRR 表示 |
